...
namespace Upsilon.Apps.Passkey.UnitTests.Multiplateform
{
   public static class UnitTestsHelper
   {
      ...
      public static void LastActivitiesShouldMatch(IDatabase database, ExpectedActivity[] expectedActivities)
      {
         ExpectedActivity[] actual = database.Activities
            .Select(_toExpected)
            .ToArray();

         _lastActivitiesShouldMatch(actual, expectedActivities);
      }

      public static void LastActivityAlertsShouldMatch(IDatabase database, ExpectedActivity[] expectedActivities)
      {
         ...
         ExpectedActivity[] actual = activityAlert!.Activities
            .Select(_toExpected)
            .ToArray();
      ...
      private static ExpectedActivity _toExpected(IActivity activity)
         => new(activity.EventType,
            activity.NeedsReview,
            Username: _normalize(activity.Username),
            ServiceName: _normalize(activity.ServiceName),
            AccountName: _normalize(activity.AccountName),
            FieldName: _normalize(activity.FieldName),
            FieldValue: _normalize(activity.FieldValue),
            ParentName: _normalize(activity.ParentName));

      private static string? _normalize(string? value)
         => string.IsNullOrEmpty(value) ? null : value;

      private static void _lastActivitiesShouldMatch(ExpectedActivity[] actualActivities, ExpectedActivity[] expectedActivities)
      {
         ExpectedActivity[] normalizedExpected = [.. expectedActivities.Select(e => e with
         {
            Username = _normalize(e.Username),
            ServiceName = _normalize(e.ServiceName),
            AccountName = _normalize(e.AccountName),
            FieldName = _normalize(e.FieldName),
            FieldValue = _normalize(e.FieldValue),
            ParentName = _normalize(e.ParentName),
         })];

         for (int i = normalizedExpected.Length - 1; i >= 0; i--)
         {
            _ = actualActivities[i].Should().Be(normalizedExpected[i],
               because: $"activity at index {i} (0 = newest)");
         }
      }
   }
}
