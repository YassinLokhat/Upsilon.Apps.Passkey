...
namespace Upsilon.Apps.Passkey.GUI.MAUI.Localization
{
   internal static class EnumDisplayHelper
   {
      ...
      private static string _formatAccountOption(string stored)
         => stored is "None" or "0" ? Strings.EnumValue_None : _formatFlags(stored, _accountOptionLabel);

      private static string _formatWarningType(string stored)
         => stored is "None" or "0" ? Strings.EnumValue_None : _formatFlags(stored, _warningTypeLabel);
