...
Hosts wire the filter through `LeakFilterConfig` (`Enabled` / `AutoUpdateFrequency` in days / `FilterPath`). The WPF host constructs `FilterPath` as `<exe>/pwned-sha1.pkbf` in code (not persisted in `config.json`) and exposes enable + frequency via `LocalLeakDatabaseEnabled` / `LocalLeakDatabaseAutoUpdateFrequency`. Attach with `PasswordFactory` / `ReloadLocalFilter`. Build and update via `HibpBloomBuilder.RunAsync` (`Utils/LeakFilter/`; incremental updates use concurrent `If-None-Match` revalidation). The WPF host may call incremental update at startup when offline use is enabled, frequency is non-zero, the `.pkbf` and `.ranges` sidecar exist, and `BuiltUtc` is older than the configured number of days — never a first full build. See [[Security]] and [[WPF Client]].
