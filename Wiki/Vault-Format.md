...
Leak checks may use a separate application-level HIBP Bloom filter (`.pkbf` + `.pkbf.ranges` sidecar) next to the executable (`pwned-ntlm.pkbf` in the WPF host). That file is **not** inside the vault ZIP. See [[Security]] and [[WPF Client]].
